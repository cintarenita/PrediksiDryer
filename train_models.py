...
for target in target_columns:
    ...
    # Simpan model dengan nama file aman
    safe_target_name = sanitize_filename(target)
    model_filename = f"{model_dir}/xgb_model_{safe_target_name}.pkl"
    joblib.dump(model, model_filename)

# Simpan evaluasi ke CSV
results_df = pd.DataFrame(results)
results_df.to_csv(f"{metrics_dir}/xgboost_metrics.csv", index=False)
print("✅ Model dan evaluasi disimpan.")